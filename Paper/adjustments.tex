...
%DIF ADD paper.tex   Tue Jun  4 21:12:46 2019
 %DIF > 
%DIF -------
...
}\DIFaddend Daarnaast is er al veel onderzoek gebeurd rond LEDs en hun \DIFdelbegin \DIFdel{mogelijk }\DIFdelend \DIFaddbegin \DIFadd{mogelijkheid }\DIFaddend tot signaaloverdracht. Zo werd er al door ETH Zurich en Disney research onderzoek gedaan naar de mogelijkheden van VLC \DIFdelbegin \DIFdel{networks }\DIFdelend \DIFaddbegin \DIFadd{netwerken (visual light communication). Dit zijn netwerken }\DIFaddend waarbij verschillende microcontrollers met elkaar \DIFdelbegin \DIFdel{communiceerden }\DIFdelend \DIFaddbegin \DIFadd{communiceren }\DIFaddend via LEDs \cite{VLCNetworks}.  \DIFaddbegin \DIFadd{Verder is er nog project firefly\mbox{%DIFAUXCMD
...
Wanneer we de gemeten waarde zonder lichtinput plotten krijgen we een duidelijk sinusodaal verloop zoals te zien op figuur \ref{fig:ruis}. Dit valt te verklaren door de RC-kringen in de gebruikte zigduino’s die als oscillator werken. \mbox{%DIFAUXCMD
\cite{oscillator}  
}\hspace{0pt}%DIFAUXCMD
}
...
\DIFadd{Voor een tweede toepassing met de na\"ieve methode wilden we de interactiemogelijkheden testen met behulp van een interactief spel. Hierbij nemen de SmartLEDs wel initiatief en wordt hun volgende actie bepaald afhankelijk van de actie van de gebruiker. 
}
...
\DIFdelbegin \DIFdel{Bij de tweede manier wordt er een bit-string verzonden naar het display. Deze manier hebben we getest met een programma waarbij een bit-string gelijk werd gesteld aan een kleur die de display LED moest aannemen of aan een programma dat afgespeeld moest }\DIFdelend \DIFaddbegin \subsection{\DIFadd{Bitstring Methode}}
...
\DIFadd{We bekijken voor elk van de drie criteria of de SmartLEDs hier geschikt voor zijn en evalueren de andere gevolgen van het gebruik van een SmartLED-display.
 }
